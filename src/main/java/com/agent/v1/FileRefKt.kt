// Generated by the protocol buffer compiler. DO NOT EDIT!
// NO CHECKED-IN PROTOBUF GENCODE
// source: agent/v1/agent.proto

@file:com.google.protobuf.Generated
// Generated files should ignore deprecation warnings
@file:Suppress("DEPRECATION")
package agent.v1;

@kotlin.jvm.JvmName("-initializefileRef")
public inline fun fileRef(block: agent.v1.FileRefKt.Dsl.() -> kotlin.Unit): agent.v1.Agent.FileRef =
  agent.v1.FileRefKt.Dsl._create(agent.v1.Agent.FileRef.newBuilder()).apply { block() }._build()
/**
 * ```
 * A file reference (attachment).
 * ```
 *
 * Protobuf type `agent.v1.FileRef`
 */
public object FileRefKt {
  @kotlin.OptIn(com.google.protobuf.kotlin.OnlyForUseByGeneratedProtoCode::class)
  @com.google.protobuf.kotlin.ProtoDslMarker
  public class Dsl private constructor(
    private val _builder: agent.v1.Agent.FileRef.Builder
  ) {
    public companion object {
      @kotlin.jvm.JvmSynthetic
    @kotlin.PublishedApi
      internal fun _create(builder: agent.v1.Agent.FileRef.Builder): Dsl = Dsl(builder)
    }

    @kotlin.jvm.JvmSynthetic
  @kotlin.PublishedApi
    internal fun _build(): agent.v1.Agent.FileRef = _builder.build()

    /**
     * `string code = 1 [json_name = "code"];`
     */
    public var code: kotlin.String
      @kotlin.jvm.JvmName("getCode")
        get() = _builder.code
      @kotlin.jvm.JvmName("setCode")
        set(value) {
        _builder.code = value
      }
    /**
     * `string code = 1 [json_name = "code"];`
     */
    public fun clearCode() {
      _builder.clearCode()
    }

    /**
     * `string name = 2 [json_name = "name"];`
     */
    public var name: kotlin.String
      @kotlin.jvm.JvmName("getName")
        get() = _builder.name
      @kotlin.jvm.JvmName("setName")
        set(value) {
        _builder.name = value
      }
    /**
     * `string name = 2 [json_name = "name"];`
     */
    public fun clearName() {
      _builder.clearName()
    }

    /**
     * `string mime = 3 [json_name = "mime"];`
     */
    public var mime: kotlin.String
      @kotlin.jvm.JvmName("getMime")
        get() = _builder.mime
      @kotlin.jvm.JvmName("setMime")
        set(value) {
        _builder.mime = value
      }
    /**
     * `string mime = 3 [json_name = "mime"];`
     */
    public fun clearMime() {
      _builder.clearMime()
    }

    /**
     * `int32 size = 4 [json_name = "size"];`
     */
    public var size: kotlin.Int
      @kotlin.jvm.JvmName("getSize")
        get() = _builder.size
      @kotlin.jvm.JvmName("setSize")
        set(value) {
        _builder.size = value
      }
    /**
     * `int32 size = 4 [json_name = "size"];`
     */
    public fun clearSize() {
      _builder.clearSize()
    }
  }
}
public inline fun agent.v1.Agent.FileRef.copy(block: `agent.v1`.FileRefKt.Dsl.() -> kotlin.Unit): agent.v1.Agent.FileRef =
  `agent.v1`.FileRefKt.Dsl._create(this.toBuilder()).apply { block() }._build()

